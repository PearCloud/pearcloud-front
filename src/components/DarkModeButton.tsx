...
export const DarkModeButton: React.FC = () => {
    ...
    const darkMode = () => {
        if (document.querySelector('html')?.classList.contains('dark')) {
            ...
            document.querySelector('meta[name="apple-mobile-web-app-status-bar-style"]')?.setAttribute("content", "#f8fafc");
            setThemeDark(false)
        } else {
            document.querySelector('html')?.classList.add('dark')
            document.querySelector('meta[name="theme-color"]')?.setAttribute("content", "#334155");
            document.querySelector('meta[name="apple-mobile-web-app-status-bar-style"]')?.setAttribute("content", "#334155");
            setThemeDark(true)
        }
    }
    return (
