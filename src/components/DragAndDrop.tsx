        ...
        <>
            <label
                ...
                onDrop={onDrop}>
                {
                    ...
                    <>
                        <BiUpload size={24} />
                        <span className="ml-4 text-md md:text-2xl text-slate-800 dark:text-gray-200">Drop here</span>
                    </>
                    :
                    <>
                        <BiUpload size={24} />
                        <span className="ml-4 text-md md:text-2xl text-slate-800 dark:text-gray-200">Choisir une ou des photos</span>
                    </>
                }
            </label>
            <input
                ...
                className="w-0 h-0 opacity-0 absolute overflow-hidden -z-10" />
            {
                fileList.length > 0 ? (
                    <div className="w-full">
                        <div className="text-center underline my-4">Prêt pour l'envoie</div>
                        <div className="w-10/12 mx-auto h-32 md:h-3/5 rounded-lg overflow-auto">
                            {
                                fileList.map((item, i) => (
                                    <div key={i} className="flex justify-between">
                                        ...
                                        <span className="cursor-pointer" onClick={() => fileRemove(item)}>
                                            <IoMdClose />
                                        </span>
                                    </div>
                                ))
